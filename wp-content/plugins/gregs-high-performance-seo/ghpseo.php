...
class gregsHighPerformanceSEO {
	...
	// whether we'll be consolidating our options into single array, or keeping discrete
	private $consolidate;

	// flag to suspend filtering of pre_get_document_title when it's our own request
	private $now_requesting_wp_title = false;
	...
	function __construct( $plugin_prefix='', $option_style='' ) {
		...
		// tap into pre_get_document_title to enable efficient delivery of modified titles
		add_filter( 'pre_get_document_title', array( $this, 'tweak_wp_generated_titles'), 1, 1 );
	...
	/**
	 * We'll hook this into pre_get_document_title to provide modified titles efficiently.
	 * @param  string $title Title passed in
	 * @return string        Modified title, or original if we shouldn't mess with it
	 */
	function tweak_wp_generated_titles( $title = '' ) {
		// just to be safe, if anyone else is messing with the title, don't interfere
		if ( !empty( $title ) )
			return $title;
		// and if this is our own request to wp_get_document_title, don't mess with it
		if ( $this->now_requesting_wp_title )
			return $title;
		return $this->select_title( true, false );
	}
	...
	function get_swaps( $type='' ) { // return replacements necessary to construct titles and descriptions
		...
		$swaps = array (
			...
			"homestaticposts" => array ( 'home_static_posts', array( "%page_title%" => ltrim( get_bloginfo( 'name' ) ), "%page_title_custom%" => $secondary ) ),
			"home" => array ( 'home', '' ),
			...
			"page" => array ( 'page', array( "%page_title%" => ltrim( single_post_title( '', false ) ), "%page_title_custom%" => $secondary ) ),
	...
	function get_comment_page_title( $ismain = false ) { // construct the title for paged comment pages
		...
		else $title = ltrim( single_post_title( '', false ) );
		return $title; // output will still need texturizing, but that's OK
	} // end getting comment page title

	// root function for titles calls on other functions to produce the title, depending on type of page; $main controls whether to return main or secondary title
	function select_title( $main = true, $echo = true ) {
		global $post;
	...
	// get titles for other than paged comments; $main controls whether to return main or secondary title
	function get_other_titles( $main = false ) {
		global $wp_query, $post;

		// provide fallback if main requested but main title mods are switched off
		if ($main && !$this->opt('enable_main_title_modifications'))
			return $this->wp_title_fallback();
		// or if secondary requested and both main and secondary mods switched off
		if (!$main && !$this->opt('enable_main_title_modifications') && !$this->opt('enable_secondary_titles'))
			return $this->wp_title_fallback();
		...
		else $title = get_bloginfo( 'name' ); // if it was none of these, just get the usual
	...
	// remove post content if we're on a paged comment page
	function paged_comments_dupefix( $content ) {
