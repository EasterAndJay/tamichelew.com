				...
				<?php if(!empty($instance['show_content'])) : ?> 				
					<p>
						...
						<?php if(!empty($instance['more_text'])) : ?>
							<a class="more-link" href="<?php the_permalink() ?>" rel="<?php _e("bookmark", "solostream"); ?>" title="<?php _e("Permanent Link to", "solostream"); ?> <?php the_title(); ?>"><?php echo esc_html( $instance['more_text'] ); ?></a>
						<?php endif; ?>
...
class Featposts_Widget extends WP_Widget {
	...
	function Featposts_Widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'featposts', 'description' => __('Adds posts with a specific tag .', "solostream") );
		/* Widget control settings. */
