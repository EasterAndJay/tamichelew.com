...
// Wrap (Page) Styles
	...
	$sitewidth = get_option('solostream_sitewidth');

	if ( $sitewidth )
		$output .= '.outer-wrap,.limit {width:'.$sitewidth.';}' . "\n";

	$wrapper = '';
	if ( $page_border_width )
		$wrapper .= 'border-width:'.$page_border_width.';border-top:0;border-bottom:0;';
	if ( $page_border_color )
		$wrapper .= 'border-style:solid;border-color:'.$page_border_color.';';
	if ( $page_box_shadow == "Yes" )
		$wrapper .= '-moz-box-shadow:0px 2px 6px rgba(100,100,100,0.4);-webkit-box-shadow:0px 2px 6px rgba(100,100,100,0.4);box-shadow:0px 2px 6px rgba(100,100,100,0.4);';
...
// Site Title Styles
	...
	$catnav = get_option('solostream_show_catnav');

	if ( $catnav == 'no' ) {
		$output .= '#header {border-bottom:3px double #e5e5e5;}' . "\n";
	}
...
// Main Content Adjustments
	...
	if ( $main_link_color )
		$output .= '#content a, #content a:link, #content a:visited,.maincontent a, .maincontent a:link, .maincontent a:visited {color:'.$main_link_color.';}' . "\n";
	if ( $main_link_hover_color )
		$output .= '#content a:hover, #content a:active.maincontent a:hover, .maincontent a:active {color:'.$main_link_hover_color.';}' . "\n";
...
// Button Colors
	...
	if ( $button_font )
		$output .= '#commentform input#submit,.archive-tabs a,.archive-tabs a:link,.archive-tabs a:visited,p.email-form .subbutton,#searchform #submitbutton {color:'.$button_font.';}' . "\n";
	if ( $button_hover_bg )
	...
	if ( $button_hover_font )
		$output .= '#commentform input#submit:hover,.archive-tabs a:hover,.archive-tabs a:active,.archive-tabs .ui-tabs-selected a,p.email-form .subbutton:hover,#searchform #submitbutton:hover {color:'.$button_hover_font.';}' . "\n";
...
// Styles for Non-Responsive Design

	if ( get_option('solostream_responsive_off') == 'Yes'  )
	$output .= '#topnav,#footer-widgets,#footer { min-width:1180px; }' . "\n";
